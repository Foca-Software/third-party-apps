...
class PaymentDecidir(http.Controller):
    ...
    @http.route([
        '/shop/cart/decidirv2_endop', 
        '/shop/cart/decidirv2_endop/<string:ref>'
    ...
    @http.route(['/shop/cart/decidirv2_make_payment'], type='json', auth="public", methods=['POST'], website=True, csrf=False)
    def decidirv2_make_payment(self, acquirer_id, token, **kwargs):
        ...
        if ('base_amount' in kwargs and 'decidir_order_id' in kwargs):
            order = request.env['sale.order'].sudo().browse(int(kwargs['decidir_order_id']))
        ...
        payment = acquirer_id.payment_decidir_create_payment(order, token, total, fees,  instalment_id, decidir_bin)
        return payment
